...
def load_htl_source(excel_bytes: bytes) -> pd.DataFrame:
    df = load_excel_sheet(excel_bytes, "HTL")
    # Remove Excel artifact columns and normalize header spacing.
...
def train_htl_models(excel_bytes: bytes, test_size: float, n_estimators: int, random_state: int) -> HtlBundle:
    df = load_htl_source(excel_bytes)
    ...
    jv_df = df[jv_cols]
    jv_long = jv_df.melt(id_vars="v(V)", var_name="Thickness", value_name="Current")
    jv_long["Thickness"] = jv_long["Thickness"].str.extract(r"(\d+)").astype(float)
    ...
    qe_df = df[qe_cols]
    qe_long = qe_df.melt(id_vars="lambda(nm)", var_name="Thickness", value_name="QE")
    qe_long["Thickness"] = qe_long["Thickness"].str.extract(r"(\d+)").astype(float)
    ...
    r_df = df[r_cols]
    r_long = r_df.melt(id_vars="lambda(nm).1", var_name="Thickness", value_name="Responsivity")
    r_long["Thickness"] = r_long["Thickness"].str.extract(r"(\d+)").astype(float)
    r_long = r_long.dropna(inplace=False)
    r_long = r_long.rename(columns={"lambda(nm).1": "Wavelength"})

    jv_long["Thickness"] = jv_long["Thickness"].astype(int)
...
def render() -> None:
    ...
    st.markdown(
        """
        <div class="hero">
            <h1>HTL Notebook App</h1>
            <p>This view mirrors htl1.ipynb: HTL J-V, QE, Responsivity, model training, predictions, and actual-vs-predicted curves.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if train_clicked:
        try:
            bundle = train_htl_models(excel_bytes, float(test_size), int(n_estimators), int(random_state))
            st.session_state["htl_bundle"] = bundle
            st.session_state["htl_train_cfg"] = {
                "test_size": float(test_size),
                "n_estimators": int(n_estimators),
                "random_state": int(random_state),
            }
            st.success("HTL models trained successfully.")
        except Exception as exc:
            st.error(f"Training failed: {exc}")
            return

    if "htl_bundle" not in st.session_state:
        st.info("Click Train Model once. After that, all inputs and graphs update live without retraining.")
        return

    bundle: HtlBundle = st.session_state["htl_bundle"]
    cfg = st.session_state.get("htl_train_cfg", {})
    ...
    st.plotly_chart(
        line_traces_by_thickness(bundle.jv_long, "Voltage", "Current", "HTL J-V Curves", "Voltage", "Current Density"),
        width="stretch",
    )
    st.plotly_chart(
        line_traces_by_thickness(bundle.qe_long, "Wavelength", "QE", "HTL QE vs Wavelength", "Wavelength (nm)", "QE"),
        width="stretch",
    )
    st.plotly_chart(
        line_traces_by_thickness(bundle.r_long, "Wavelength", "Responsivity", "HTL Responsivity vs Wavelength", "Wavelength (nm)", "Responsivity"),
        width="stretch",
    )
    ...
    with v1:
        st.plotly_chart(
            scatter_validation(bundle.y_test_jv, bundle.y_pred_jv, "HTL Model Validation (J-V)", "Actual Current", "Predicted Current"),
            width="stretch",
        )
    with v2:
        st.plotly_chart(
            scatter_validation(bundle.y_test_qe, bundle.y_pred_qe, "HTL QE Validation", "Actual QE", "Predicted QE"),
            width="stretch",
        )
    with v3:
        st.plotly_chart(
            scatter_validation(bundle.y_test_r, bundle.y_pred_r, "HTL Responsivity Validation", "Actual R", "Predicted R"),
            width="stretch",
        )
    ...
    jv_actual = bundle.jv_long[bundle.jv_long["Thickness"] == compare_thickness].sort_values("Voltage")
    if not jv_actual.empty:
        ...
        st.plotly_chart(
            compare_curve(jv_actual["Voltage"], jv_actual["Current"], jv_x, jv_pred, f"J-V Comparison ({compare_thickness} nm HTL)", "Voltage", "Current"),
            width="stretch",
        )
    else:
    ...
    if not qe_actual.empty:
        ...
        st.plotly_chart(
            compare_curve(qe_actual["Wavelength"], qe_actual["QE"], qe_x, qe_pred, f"QE Comparison ({compare_thickness} nm HTL)", "Wavelength (nm)", "QE"),
            width="stretch",
        )
    else:
    ...
    if not r_actual.empty:
        ...
        st.plotly_chart(
            compare_curve(r_actual["Wavelength"], r_actual["Responsivity"], r_x, r_pred, f"Responsivity Comparison ({compare_thickness} nm HTL)", "Wavelength (nm)", "Responsivity"),
            width="stretch",
        )
    else:
        st.info("No actual Responsivity data for this thickness.")

    st.subheader("Detailed Physical Interpretation - HTL")
    with st.expander("Show HTL Theory", expanded=False):
        st.markdown(
            """
1. HTL is responsible for hole extraction and transport to the contact.
2. Thickness tuning changes transport resistance and recombination probability.
3. Very thin HTL can hurt selectivity and extraction; very thick HTL can increase series resistance.
4. J-V, QE, and Responsivity trends together show how transport losses evolve with HTL thickness.
5. This dashboard keeps HTL workflow isolated from ETL, PVK, and Temperature pipelines.

Conclusion: HTL thickness optimization is critical for balancing charge extraction and resistive losses.
            """
        )

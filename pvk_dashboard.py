...
def load_pvk_source(excel_bytes: bytes) -> pd.DataFrame:
    return load_excel_sheet(excel_bytes, "PVK")


@st.cache_resource(show_spinner=False)
def train_pvk_models(excel_bytes: bytes, test_size: float, n_estimators: int, random_state: int) -> PvkBundle:
    df = load_pvk_source(excel_bytes)
...
def render() -> None:
    ...
    st.markdown(
        """
        <div class="hero">
            <h1>PVK Notebook App</h1>
            <p>This view mirrors pvk1.ipynb: PVK J-V, QE, Responsivity, model training, predictions, and actual-vs-predicted curves.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if train_clicked:
        try:
            bundle = train_pvk_models(excel_bytes, float(test_size), int(n_estimators), int(random_state))
            st.session_state["pvk_bundle"] = bundle
            st.session_state["pvk_train_cfg"] = {
                "test_size": float(test_size),
                "n_estimators": int(n_estimators),
                "random_state": int(random_state),
            }
            st.success("PVK models trained successfully.")
        except Exception as exc:
            st.error(f"Training failed: {exc}")
            return

    if "pvk_bundle" not in st.session_state:
        st.info("Click Train Model once. After that, all inputs and graphs update live without retraining.")
        return

    bundle: PvkBundle = st.session_state["pvk_bundle"]
    cfg = st.session_state.get("pvk_train_cfg", {})
    ...
    st.plotly_chart(
        line_traces_by_thickness(bundle.jv_long, "Voltage", "Current", "PVK J-V Curves", "Voltage", "Current Density"),
        width="stretch",
    )
    st.plotly_chart(
        line_traces_by_thickness(bundle.qe_long, "Wavelength", "QE", "PVK QE vs Wavelength", "Wavelength (nm)", "QE"),
        width="stretch",
    )
    st.plotly_chart(
        line_traces_by_thickness(bundle.r_long, "Wavelength", "Responsivity", "PVK Responsivity vs Wavelength", "Wavelength (nm)", "Responsivity"),
        width="stretch",
    )
    ...
    with v1:
        st.plotly_chart(
            scatter_validation(bundle.y_test_jv, bundle.y_pred_jv, "PVK Model Validation (J-V)", "Actual Current", "Predicted Current"),
            width="stretch",
        )
    with v2:
        st.plotly_chart(
            scatter_validation(bundle.y_test_qe, bundle.y_pred_qe, "PVK QE Validation", "Actual QE", "Predicted QE"),
            width="stretch",
        )
    with v3:
        st.plotly_chart(
            scatter_validation(bundle.y_test_r, bundle.y_pred_r, "PVK Responsivity Validation", "Actual R", "Predicted R"),
            width="stretch",
        )
    ...
    if not jv_actual.empty:
        ...
        st.plotly_chart(
            compare_curve(jv_actual["Voltage"], jv_actual["Current"], jv_x, jv_pred, f"J-V Comparison ({compare_thickness} nm PVK)", "Voltage", "Current"),
            width="stretch",
        )
    else:
    ...
    if not qe_actual.empty:
        ...
        st.plotly_chart(
            compare_curve(qe_actual["Wavelength"], qe_actual["QE"], qe_x, qe_pred, f"QE Comparison ({compare_thickness} nm PVK)", "Wavelength (nm)", "QE"),
            width="stretch",
        )
    else:
    ...
    if not r_actual.empty:
        ...
        st.plotly_chart(
            compare_curve(r_actual["Wavelength"], r_actual["Responsivity"], r_x, r_pred, f"Responsivity Comparison ({compare_thickness} nm PVK)", "Wavelength (nm)", "Responsivity"),
            width="stretch",
        )
    else:
        st.info("No actual Responsivity data for this thickness.")

    st.subheader("Detailed Physical Interpretation - PVK")
    with st.expander("Show PVK Theory", expanded=False):
        st.markdown(
            """
1. PVK is the absorber layer, so it directly controls light harvesting and carrier generation.
2. Thickness changes both absorption depth and recombination behavior.
3. Too thin can reduce absorption; too thick can increase recombination and transport losses.
4. J-V, QE, and Responsivity all respond strongly to absorber thickness because the active layer is the source of photocarriers.
5. The PVK dashboard mirrors the notebook workflow but stays isolated from ETL code.

Conclusion: PVK thickness is a primary design variable for optical absorption and spectral response.
            """
        )